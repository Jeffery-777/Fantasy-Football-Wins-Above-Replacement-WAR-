...
This Repo takes fantasy football weekly score output for all relevant players from 2010 - 2019 and calculates the wins above replacement for each player relative to that player's position. 

```{r message=FALSE, warning=FALSE}
library(tidyverse)
data <- readRDS("Half_FF_Complete_WAR.rds") # This data was scraped from fantasyfootballdb.com

# Points were preset in database for two positions (K and DST) but the rest need to be calculated as performed below to make 'df'
...
# Now I have all the points for each week of each player properly calibrated for my scoring settings
```


# Funtion to calculate team averages and war for all years 

Below chunk creates baselines for each position in each year. Most leagues use 12 teams, so 12th rank at end of year is presumed the baseline for single starting roster positions and 24 for double rostered positions.
...
# Flex is treated differently because it's not a native position - it's a combo of either WR/RB/TE. So to get the average flex score, the remaining players left that haven't been assigned to their native positions are flex eligible and the top 12 fuel the calculations.
...
flex_figures <- bind_rows(rbflx_names, wrflx_names, teflx_names) %>% 
  left_join(df, by = c("player","season" = "year")) %>% 
...
# slight changes for k and dst - to account for streamability of these positions, instead of basing top 12 off seasonal results, average and standard deviation are calculated based on top 12 of each week
...
k_figures <- df %>% 
  ...
  # now with all kickers since 2010 ordered in year, week and top pts scored, filter the top 12 of each year/week and get avg
  ungroup() %>% 
  group_by(year, week) %>% 
  top_n(n = 12, wt = total_pts) %>% 
  # now with all top 12 finishes of each year and week, ungroup and group by year only to get annual metrics
  ungroup() %>% 
  group_by(year) %>% 
  summarise(mean = mean(total_pts, na.rm = T), sd = sd(total_pts, na.rm = T))
  
# Repeat process for DST
...
avg_team_sd <- map(.x = c(1:10),
                   .f = function(x) {
                    avg_team_sd <- sqrt((qb_figures$sd[x] ^ 2) + 
                          (rb_figures$sd[x] ^2) + 
                           (rb_figures$sd[x] ^ 2) +
                           (wr_figures$sd[x] ^ 2) +
                           (wr_figures$sd[x] ^2) +
...
dst_top_12 <- df %>% 
  ...
  summarise(total_pts = sum(half)) %>% 
  arrange(desc(year), week, desc(total_pts)) %>% 
  # now with all dsts since 2010 ordered in year, week and top pts scored, filter the top 12 of each year/week and get avg
  ungroup() %>% 
  rename(season = year) %>% 
...
all_replacement <-  bind_rows(qb_repl_names, 
                              rb_repl_names, 
                              wr_repl_names, 
                              te_repl_names, 
                              dst_top_12, 
...
# Finally, create the dataframe with the WAR results
war_final <- war %>% 
  ...
  ungroup() %>%
  # To limit one week wonders or the like a 6 game min is used for eligibility
  filter(games >= 6) %>% 
  arrange(desc(WAR)) %>% 
  mutate(rank = row_number(),
         player = str_replace(player, "Odell Beckham.*", "Odell Beckham")) # fix a funny name spelling
